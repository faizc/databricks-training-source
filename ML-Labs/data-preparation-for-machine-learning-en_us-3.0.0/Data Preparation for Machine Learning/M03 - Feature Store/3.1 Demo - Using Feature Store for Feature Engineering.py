...
# MAGIC
# MAGIC In this demo, we will guide you to explore the use of Feature Stores to enhance the feature engineering workflow and understand its crucial role in the development of machine learning models. First we will create feature store tables for effective implementation in feature engineering processes and then discuss how to update features. Also, we will cover how to convert an existing table to a feature table in Unity Catalog.
# MAGIC
...
# MAGIC
# MAGIC We can also look at the metadata of the feature store via the FeatureStore client by using **`get_table()`**. *As a feature table is a Delta table, we can load it with Spark as we normally do for other tables*.
...
# MAGIC
# MAGIC In some cases we might need to update an existing feature table by adding new features or deleting existing features. In this section, we will show how to make these types of changes.
